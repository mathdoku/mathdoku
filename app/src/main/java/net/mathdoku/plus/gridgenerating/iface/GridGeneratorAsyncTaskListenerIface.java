...
public interface GridGeneratorAsyncTaskListenerIface {
	/**
	 * Inform the listener if a grid is generated. This event is only sent in
	 * case multiple grids have to be generated.
	 */
	void onGridGenerated();

	/**
	 * Inform the listener when the grid generator has finished generating the
	 * grid(s).
	 * 
	 * @param grids
	...
	/**
	 * Inform the listener if the grid generating task has been cancelled by the
	 * async task or its super class.
	 */
