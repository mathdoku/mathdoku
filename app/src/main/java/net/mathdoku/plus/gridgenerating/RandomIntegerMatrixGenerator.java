...
/**
 * Creates an Integer Matrix filled with random values. Each number from 1 to
 * the maximum value is used exactly once in each row and exactly once in each
 * column.
 */
public class RandomIntegerMatrixGenerator {
	...
	private void randomizeValue(int value) {
		...
		throw new GridGeneratingException(String.format(
				"Too many tries to randomize value %d.", value));
	}
	...
	/**
	 * Finds a random column in the given row in which the given value can be
	 * place without violation the constraint that the value is already used in
	 * the column.
	 */
	private int getRandomEligibleColumnOnRowForValue(int row, int value) {
		...
		for (int attempts = 1; attempts <= MAX_ATTEMPTS_TO_GET_AN_ELIGIBLE_COLUMN_FOR_A_ROW; attempts++) {
			...
			if (integerMatrix.isEmpty(row, column)
					&& !integerMatrix.getColumn(column).contains(value)) {
				return column;
			}
		}
		return -1;
