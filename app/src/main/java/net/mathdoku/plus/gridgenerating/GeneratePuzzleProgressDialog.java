...
public class GeneratePuzzleProgressDialog implements GridGeneratorAsyncTaskListenerIface {
    ...
    /**
     * The activity used to display the dialog, and to forward the generated grid to.
     */
    ...
    private ProgressDialog buildDialog() {
        ...
        progressDialog.setMessage(mPuzzleFragmentActivity.getResources()
        ...
        // Set style of dialog.
        if (Config.APP_MODE == AppMode.DEVELOPMENT && numberOfGamesToGenerate > 1) {
    ...
    public void onFinishGridGenerator(List<Grid> grids) {
        ...
        if (grids.size() == 1 && mPuzzleFragmentActivity != null) {
            ...
            DevelopmentHelper.generateGamesReady(mPuzzleFragmentActivity, grids.size());
        }
    }
    ...
    public void onCancelGridGeneratorAsyncTask() {
        if (mPuzzleFragmentActivity != null) {
            if (DEBUG) {
                Log.d(TAG, "Inform activity about cancellation of the grid generation.");
            }
