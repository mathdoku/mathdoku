 ...
 * other elements have been chosen before.
 * 
 * Each item has a certain weight which affects the chance of the item to be
 * chose. A bigger weight increases the chance the item is chosen.
 * 
 * 
 * @param <T>
...
public class RandomWeightedListItemSelector<T> {
	...
	public RandomWeightedListItemSelector(Random random,
			List<WeightedItem<T>> list) {
		if (random == null) {
			throw new IllegalArgumentException(
					"Cannot create instance when random is 'null'.");
		}
		if (list == null) {
			throw new IllegalArgumentException(
					"Cannot create instance when list is 'null'.");
		}
		if (list.isEmpty()) {
			throw new IllegalArgumentException(
					"Cannot create instance when list is empty.");
		}
		this.random = random;
	...
	/**
	 * Sets the item which is chosen only when all other items have been chosen
	 * before.
	 */
	...
	private void removeWeightedItem(T lastItem) {
		...
		while (iterator.hasNext()) {
			...
			if (weightedItem.getItem().equals(lastItem)) {
				iterator.remove();
			}
		}
	}
		...
		if (list.isEmpty()) {
			if (lastItemToBeSelected == null) {
				throw new IllegalStateException(
						"List is empty. Last item to be selected (if applicable) has already been returned.");
			}
