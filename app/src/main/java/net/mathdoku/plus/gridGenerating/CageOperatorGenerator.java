...
class CageOperatorGenerator {
    ...
    private CageResult selectRandomCageOperator() {
        ...
        for (Map.Entry<CageResult, Integer> entry : possibleCageResults.entrySet()) {
    ...
    private void addPossibleCageResults() {
        ...
        addPossibleCageResult(CageOperator.DIVIDE, WEIGHT_CAGE_RESULT_WITH_DIVIDE_OPERATOR);
        addPossibleCageResult(CageOperator.SUBTRACT, WEIGHT_CAGE_RESULT_WITH_SUBTRACT_OPERATOR);
        addPossibleCageResultMultiply(WEIGHT_CAGE_RESULT_WITH_MULTIPLY_OPERATOR);
        addPossibleCageResult(CageOperator.ADD, WEIGHT_CAGE_RESULT_WITH_ADD_OPERATOR);
    }
    ...
    private void addPossibleCageResult(CageOperator cageOperator, int weight) {
        if (CageResult.canBeCreated(cageOperator, cellValues)) {
            CageResult possibleCageResult = CageResult.create(cageOperator, cellValues);
