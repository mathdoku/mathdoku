...
class CageDefinitionSplitter {
	...
	public CageDefinitionSplitter(String cageDefinition) {
		...
		cageDefinitionElements = cageDefinition
				.split(GridDefinitionDelimiter.LEVEL2);
		if (cageDefinitionElements == null
				|| cageDefinitionElements.length != 3) {
			throw new InvalidGridException(String.format(
					"Cage definition '%s' has invalid number of elements.",
					cageDefinition));
		}
	}
	...
	public CageOperator getCageOperator() {
		...
		try {
			cageOperator = CageOperator
					.fromId(cageDefinitionElements[CAGE_OPERATOR_ID]);
		} catch (IllegalArgumentException e) {
			throw new InvalidGridException(String.format(
					"Invalid cage operator '%s' in cage definition '%s'.",
					cageDefinitionElements[CAGE_OPERATOR_ID], cageDefinition),
					e);
		}
