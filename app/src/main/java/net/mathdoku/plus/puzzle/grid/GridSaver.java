...
public class GridSaver {
    ...
    public boolean save(Grid grid) {
        ...
        mIsSaved = saveGridRow(grid) && saveSolvingAttempt(grid) && saveStatistics(grid);
        if (mIsSaved) {
    ...
    private boolean saveGridRow(Grid grid) {
        ...
        if (mRowId < 0) {
            ...
            GridDatabaseAdapter gridDatabaseAdapter = mObjectsCreator.createGridDatabaseAdapter();
            GridRow gridRow = gridDatabaseAdapter.getByGridDefinition(gridDefinition);
            mRowId = gridRow == null ? gridDatabaseAdapter.insert(grid) : gridRow.getGridId();
        }
    ...
    private boolean saveSolvingAttempt(Grid grid) {
        ...
        if (mSolvingAttemptId < 0) {
            solvingAttemptRow = solvingAttemptDatabaseAdapter.insert(solvingAttemptRow);
    ...
    private boolean saveStatistics(Grid grid) {
        ...
        if (mGridStatistics.mId < 0) {
            ...
            mGridStatistics.mId = statisticsDatabaseAdapter.insert(mGridStatistics);
