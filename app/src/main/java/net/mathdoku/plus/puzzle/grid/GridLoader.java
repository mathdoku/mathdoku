...
/**
 * The GridLoad is responsible for loading a grid from the database into a new {@link Grid} object.
 */
public class GridLoader {
     ...
     * @return The Grid if load is successfully completed. Null in case of an error.
     */
    public Grid load(int solvingAttemptId) {
        ...
        mGridBuilder.setGridSize(gridRow.getGridSize())
     ...
     *
     * @return True in case the entire string is processed successful. False otherwise.
     */
    ...
    private boolean loadFromStorageStrings(SolvingAttemptRow solvingAttemptRow) {
        if (solvingAttemptRow.getStorageString() == null) {
            return errorOnLoadStorageString("Solving attempt contains no storage string.", null);
        }
    ...
    private boolean loadCellChange(String line) {
        ...
        CellChangeStorage cellChangeStorage = mObjectsCreator.createCellChangeStorage();
        if (!cellChangeStorage.fromStorageString(line, mCells, mSavedWithRevision)) {
            return false;
        }
        CellChange cellChange = mObjectsCreator.createCellChange(cellChangeStorage);
