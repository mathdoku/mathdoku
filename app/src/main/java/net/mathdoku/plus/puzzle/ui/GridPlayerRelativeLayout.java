...
public class GridPlayerRelativeLayout extends RelativeLayout {
    ...
    public GridPlayerRelativeLayout(Context context, AttributeSet attrs) {
        ...
        TypedArray typedArray = context.getTheme()
                .obtainStyledAttributes(attrs, R.styleable.GridPlayerViewLayoutAlign, 0, 0);
        if (typedArray != null) {
            try {
        ...
        if (mMarginAdjustment) {
            ...
            if (!mMarginsInitialised) {
                ...
                if (mLeftMarginAdjustment) {
                    mLeftMargin = (int) Painter.getInstance()
                            .getCellPainter()
                            .getCellSize() / 2;
                }
                if (mTopMarginAdjustment) {
                    mTopMargin = (int) Painter.getInstance()
                            .getCellPainter()
                            .getCellSize() / 2;
                }
                if (mRightMarginAdjustment) {
                    mRightMargin = (int) Painter.getInstance()
                            .getCellPainter()
                            .getCellSize() / 2;
                }
