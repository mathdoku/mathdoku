...
public class SwipeMotion extends Motion {
     ...
     *         The border width in the grid view in which the swipe motion is created.
     * @param cellSize
     *         The size of the cells in the grid view in which the swipe motion is created.
     */
    SwipeMotion(GridPlayerView gridPlayerView, float gridViewBorderWidth, float cellSize) {
     ...
     *
     * @param event
     *         The event which is registered as the touch down event of the swipe motion.
     */
    @Override
    ...
    void release(MotionEvent event) {
        if (mStatus == Status.INIT) {
            ...
            throw new IllegalStateException(
                    getErrorIllegalStateTransition(mStatus.toString(), Status.RELEASED.toString()));
        }
     ...
     *
     * @return True in case the motion has been released but not yet completed. False otherwise.
     */
    boolean isReleased() {
    ...
    void finish() {
        ...
        } else if (Config.APP_MODE == AppMode.DEVELOPMENT && mStatus != Status.FINISHED) {
            throw new IllegalStateException(
                    getErrorIllegalStateTransition(mStatus.toString(), Status.FINISHED.toString()));
        }
    }
    ...
    boolean update(MotionEvent motionEvent) {
        ...
        if (inTouchDownCell) {
            if (mStatus != Status.RELEASED || mGridSize < 7 ||
                ...
                if (mListener != null && mCurrentSwipePositionDigit != DIGIT_UNDETERMINED) {
     ...
     *
     * @return True in case the current swipe position needs to be updated. False otherwise.
     */
    ...
    /**
     * Get the angle to the middle of the segment which is used to select the given digit.
     *
     * @param digit
     *         The digit for which the middle of the swipe segment has to be determined.
     * @return The angle to the middle of the segment which is used to select the given digit.
     */
    public static int getAngleCenterSwipeSegment(int digit) {
        return SWIPE_ANGLE_OFFSET_91 + (digit - 1) * SWIPE_SEGMENT_ANGLE + SWIPE_SEGMENT_ANGLE / 2;
    }
