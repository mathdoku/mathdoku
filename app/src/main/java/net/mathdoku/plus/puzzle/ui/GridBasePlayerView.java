...
/**
 * The grid base player view allows to play a grid with a digit button interface only.
 */
public class GridBasePlayerView extends GridViewerView implements OnTouchListener {
    ...
    public interface OnInputModeChangedListener {
        public abstract void onInputModeChanged(GridInputMode inputMode, boolean enableCopyMode);
    }
     ...
     *
     * @param newValue
     *         The value which has to be entered/removed from the selected cell in the grid.
     */
    public void digitSelected(int newValue) {
        assert mInputMode == GridInputMode.NORMAL || mInputMode == GridInputMode.MAYBE ||
        ...
        if (newValue == 0) { // Clear Button
            if (!selectedCell.isEmpty()) {
                ...
                mGrid.getGridStatistics()
                        .increaseCounter(StatisticsCounterType.ACTION_CLEAR_CELL);
            ...
            if (TipOrderOfValuesInCage.toBeDisplayed(mPreferences, mGrid.getSelectedCage())) {
    ...
    /**
     * Copies the user value or the possible values from one cell to another cell.
     *
     * @param fromCell
    ...
    /**
     * Check the validity of the grid after a user value has been set for the given grid cell.
     *
     * @param selectedCell
    ...
    public void setCopyModeEnabled(boolean enableCopyMode) {
        ...
        if (enableCopyMode) {
            ...
            if (mInputMode == GridInputMode.NORMAL || mInputMode == GridInputMode.MAYBE) {
    ...
    public void setGridInputMode(GridInputMode gridInputMode, boolean enableCopyMode) {
        mInputMode = gridInputMode;
        invalidate();

        // Inform listeners about change in input mode
        if (mOnInputModeChangedListener != null) {
            mOnInputModeChangedListener.onInputModeChanged(mInputMode, enableCopyMode);
        }
    }
     ...
     */
    public void setOnInputModeChangedListener(OnInputModeChangedListener onInputChangedModeListener) {
     ...
     *
     * @param listener
     *         The listener which is to be called in case the gird has been touched.
     */
