...
public class TickerTape extends HorizontalScrollView {
    ...
    public TickerTape(Context context, AttributeSet attrs) {
        ...
        int textSizeInPixels = getResources().getDimensionPixelSize(R.dimen.text_size_default);
    ...
    String getCurrentItem() {
        return textViewList.isEmpty() ? null : textViewList.get(0)
                .getText()
                .toString();
    }
     ...
     *
     * @return True is the next scroll position is valid for the current item. False otherwise.
     */
    private boolean scrollToNextPositionInCurrentItem() {
        ...
        return newScrollPos <= textViewList.get(0)
                .getWidth() && getScrollX() >= newScrollPos;
    }
     ...
     *         The minimum amount of milliseconds the ticker tape has to be displayed.
     * @return The ticker tape object itself so it can be used as a builder.
     */
    @SuppressWarnings("SameParameterValue")
    public TickerTape setEraseConditions(int minDisplayCycles, long minDisplayTime) {
    ...
    /**
     * Resets the ticker tape. All messages will be cleared and the ticker tape is hidden.
     *
     * @return The ticker tape object itself so it can be used as a builder.
     */
