...
public class StatisticsDatabaseAdapter extends DatabaseAdapter {
    ...
    private static DatabaseTableDefinition defineTable() {
        ...
        databaseTableDefinition.addColumn(
                new DatabaseColumnDefinition(KEY_ACTION_CLEAR_CELL, DataType.INTEGER).setNotNull()
                        .setDefaultValue(0));
        databaseTableDefinition.addColumn(
                new DatabaseColumnDefinition(KEY_ACTION_CLEAR_GRID, DataType.INTEGER).setNotNull()
                        .setDefaultValue(0));
        databaseTableDefinition.addColumn(
    ...
    public int insert(GridStatistics gridStatistics) {
        ...
        try {
            ...
            throw new DatabaseAdapterException("Cannot insert new grid statistics in database.", e);
        }
    ...
    public GridStatistics getStatisticsForSolvingAttempt(int solvingAttemptId) {
        ...
        try {
            cursor = sqliteDatabase.query(true,
                                          ...
                                          null,
                                          //
                                          new OrderByHelper().sortDescending(KEY_ROWID)
                                                  .toString(),
                                          //
                                          null);
    ...
    private int getCheckProgressInvalidCellsFoundColumnFromCursor(Cursor cursor) {
        return cursor.getColumnIndexOrThrow(KEY_CHECK_PROGRESS_INVALID_CELLS_FOUND);
    }

    private String getSolutionRevealedStatusColumnFromCursor(Cursor cursor) {
        return cursor.getString(cursor.getColumnIndexOrThrow(KEY_ACTION_REVEAL_SOLUTION));
    }

    private String getSolvedManuallyStatusColumnFromCursor(Cursor cursor) {
        return cursor.getString(cursor.getColumnIndexOrThrow(KEY_SOLVED_MANUALLY));
    }
    ...
    private String getIncludeInStatisticsColumnFromCursor(Cursor cursor) {
        return cursor.getString(cursor.getColumnIndexOrThrow(KEY_INCLUDE_IN_STATISTICS));
    }
    ...
    public boolean update(GridStatistics gridStatistics) {
        ...
        newValues.put(KEY_USER_VALUES_REPLACED, gridStatistics.mEnteredValueReplaced);
    ...
    private String getDerivationNewValueIncludeInStatistics(int statisticsIdToBeIncluded) {
        ...
        stringBuilder.append(getStatisticsIdSelectionString(statisticsIdToBeIncluded));
