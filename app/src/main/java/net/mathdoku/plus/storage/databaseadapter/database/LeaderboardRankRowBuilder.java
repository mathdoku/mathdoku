...
public class LeaderboardRankRowBuilder {
	...
	/**
	 * Constructs a builder for a new leaderboard rank which is not yet stored
	 * in the database.
	 */
	public LeaderboardRankRowBuilder(String leaderboardId, int gridSize,
			boolean operatorsHidden, PuzzleComplexity puzzleComplexity) {
		this(-1, leaderboardId, gridSize, operatorsHidden, puzzleComplexity);
	}

	/**
	 * Constructs a builder for a new leaderboard rank which is already stored
	 * in the database.
	 */
	public LeaderboardRankRowBuilder(int id, String leaderboardId,
	 ...
	 */
	public static LeaderboardRankRowBuilder from(
	 ...
	 */
	public static LeaderboardRankRowBuilder from(
			LeaderboardRankRow leaderboardRankRow, int newId) {
		...
		leaderboardRankRowBuilder.scoreOrigin = leaderboardRankRow
				.getScoreOrigin();
		leaderboardRankRowBuilder.statisticsId = leaderboardRankRow
				.getStatisticsId();
		leaderboardRankRowBuilder.rawScore = leaderboardRankRow.getRawScore();
		leaderboardRankRowBuilder.dateSubmitted = leaderboardRankRow
				.getDateSubmitted();
		leaderboardRankRowBuilder.rankStatus = leaderboardRankRow
				.getRankStatus();
		leaderboardRankRowBuilder.rank = leaderboardRankRow.getRank();
		leaderboardRankRowBuilder.rankDisplay = leaderboardRankRow
				.getRankDisplay();
		leaderboardRankRowBuilder.dateLastUpdated = leaderboardRankRow
				.getDateLastUpdated();
	 ...
	 */
	public LeaderboardRankRowBuilder setScore(
	 ...
	 * @return The updated leaderboard rank row builder.
	 */
	public LeaderboardRankRowBuilder setScoreLocal(int statisticsId,
			long rawScore) {
		setScore(LeaderboardRankDatabaseAdapter.ScoreOrigin.LOCAL_DATABASE,
				statisticsId, rawScore, System.currentTimeMillis());
		setDefaultRankingInformation();
	 ...
	 * @return The updated leaderboard rank row builder.
	 */
	public LeaderboardRankRowBuilder setScoreAndRank(long rawScore, long rank,
			String rankDisplay) {
		long timestamp = System.currentTimeMillis();
		setScore(LeaderboardRankDatabaseAdapter.ScoreOrigin.EXTERNAL, 0,
				rawScore, timestamp);
		setRank(LeaderboardRankDatabaseAdapter.RankStatus.TOP_RANK_UPDATED,
				rank, rankDisplay, timestamp);
	...
	public LeaderboardRankRowBuilder setRank(
			LeaderboardRankDatabaseAdapter.RankStatus rankStatus, long rank,
	...
	public LeaderboardRankRowBuilder setRankNotAvailable() {
		setRank(LeaderboardRankDatabaseAdapter.RankStatus.TOP_RANK_NOT_AVAILABLE,
