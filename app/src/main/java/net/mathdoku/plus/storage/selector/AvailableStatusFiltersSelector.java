...
/**
 * This class selects all StatusFilters which are applicable when grids with a
 * given GridType are selected.
 */
public class AvailableStatusFiltersSelector extends SolvingAttemptSelector {
	...
	private String getProjectionStatusFilterFromCursor(Cursor cursor) {
		return cursor.getString(cursor
				.getColumnIndexOrThrow(KEY_PROJECTION_STATUS_FILTER));
	}

	protected DatabaseProjection getDatabaseProjection() {
		DatabaseProjection databaseProjection = new DatabaseProjection();
		databaseProjection.put(KEY_PROJECTION_STATUS_FILTER,
				getStatusFilterDerivationSQL());
		...
		for (SolvingAttemptStatus solvingAttemptStatus : SolvingAttemptStatus
			...
			if (solvingAttemptStatus.getAttachedToStatusFilter() != null) {
				...
				stringBuilder.append(DatabaseUtil
						.stringBetweenQuotes(solvingAttemptStatus
