...
public class GridStorage {
    ...
    private String[] getViewParts(String line, int savedWithRevisionNumber) {
        String[] viewParts = line.split(StorageDelimiter.FIELD_DELIMITER_LEVEL1);

        // Only process the storage string if it starts with the correct
        // identifier.
        if (viewParts == null || !SAVE_GAME_GRID_LINE.equals(viewParts[0])) {
            throw new StorageException(String.format("Invalid grid storage string '%s'.", line));
        }

        int expectedNumberOfElements = savedWithRevisionNumber <= 595 ? 4 : 3;
        if (viewParts.length != expectedNumberOfElements) {
            throw new InvalidParameterException("Wrong number of elements in grid storage string");
        }
    ...
    /**
     * Create a string representation of the Grid which can be used to store a grid.
     *
    ...
    public String toStorageString(Grid grid) {
        ...
        for (Cell cell : mCells) {
            stringBuilder.append(cellStorage.toStorageString(cell))
                    .append(StorageDelimiter.EOL_DELIMITER);
        }
        ...
        if (mCages != null) {
            for (Cage cage : mCages) {
                stringBuilder.append(mObjectsCreator.createCageStorageString(cage))
                        .append(StorageDelimiter.EOL_DELIMITER);
            }
        }
        ...
        if (mCellChanges != null) {
            for (CellChange cellChange : mCellChanges) {
                stringBuilder.append(cellChangeStorage.toStorageString(cellChange))
                        .append(StorageDelimiter.EOL_DELIMITER);
            }
        }
