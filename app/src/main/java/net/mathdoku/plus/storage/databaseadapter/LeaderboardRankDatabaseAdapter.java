...
public class LeaderboardRankDatabaseAdapter extends DatabaseAdapter {
    ...
    /**
     * Inserts a leaderboard rank row into the database. The given row id field is ignored.
     *
     * @param leaderboardRankRow
     *         The leaderboard rank row to be inserted.
     * @return The leaderboard rank row which is actually inserted including the row id.
     */
    ...
    private ContentValues getContentValues(LeaderboardRankRow leaderboardRankRow) {
        ...
        contentValues.put(KEY_SCORE_ORIGIN, leaderboardRankRow.getScoreOrigin()
                .toString());
        contentValues.put(KEY_SCORE_STATISTICS_ID, leaderboardRankRow.getStatisticsId());
        contentValues.put(KEY_SCORE_RAW_SCORE, leaderboardRankRow.getRawScore());
        contentValues.put(KEY_SCORE_DATE_SUBMITTED,
                          DatabaseUtil.toSQLiteTimestamp(leaderboardRankRow.getDateSubmitted()));
        contentValues.put(KEY_RANK_STATUS, leaderboardRankRow.getRankStatus()
                .toString());
        contentValues.put(KEY_RANK, leaderboardRankRow.getRank());
        contentValues.put(KEY_RANK_DISPLAY, leaderboardRankRow.getRankDisplay());
        contentValues.put(KEY_RANK_DATE_LAST_UPDATED,
                          DatabaseUtil.toSQLiteTimestamp(leaderboardRankRow.getDateLastUpdated()));
     ...
     *
     * @param leaderboardId
     *         The leaderboard id of the leaderboard which has to be retrieved.
    ...
    private String getLeaderboardSelectionString(String leaderboardId) {
        return new FieldOperatorStringValue(KEY_LEADERBOARD_ID, FieldOperatorValue.Operator.EQUALS,
    ...
    private String getLeaderboardIdFromCursor(Cursor cursor) {
        return cursor.getString(cursor.getColumnIndexOrThrow(KEY_LEADERBOARD_ID));
    }
    ...
    private int getStatisticsIdFromCursor(Cursor cursor) {
        return cursor.getInt(cursor.getColumnIndexOrThrow(KEY_SCORE_STATISTICS_ID));
    }

    private long getRawScoreFromCursor(Cursor cursor) {
        return cursor.getLong(cursor.getColumnIndexOrThrow(KEY_SCORE_RAW_SCORE));
    }
    ...
    /**
     * Get the most outdated leaderboard for which the ranking information needs to be updated.
     *
     * @return The (first/next) leaderboard for which the ranking information needs to be updated.
     */
    ...
    /**
     * Get the number of leaderboards for which the ranking information needs to be updated.
     *
     * @return The number of leaderboards for which the leaderboards needs to be updated again.
     */
    ...
    /**
     * Get the selection clause for the queries retrieving the outdated leaderboard ranks.
     *
     * @return The selection clause for the queries retrieving the outdated leaderboard ranks.
     */
    ...
    public void setAllRanksToBeUpdated() {
        try {
            ...
            throw new DatabaseAdapterException("Cannot set ranks to be updated in database", e);
        }
    }
}
