...
class TickerTapeUpdater implements Runnable {
	...
	public void run() {
		if (!isCancelled) {
			...
			countItemsDisplayedCompletely += currentItemBeforeUpdate
					.equals(tickerTape.getCurrentItem()) ? 0 : 1;

			if (tickerTape.isEraseConditionFulfilled(countItemsDisplayedCompletely, System.currentTimeMillis()
							- startTime)) {
				tickerTape.hide();
				return;
			}
	 ...
	 * 
	 * @return True in case the runnable will be cancelled on the next occasion.
	 *         False otherwise.
	 */
