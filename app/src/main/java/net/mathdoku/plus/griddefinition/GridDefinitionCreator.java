 ...
 * information about how it was created or about the current status of solving.
 * This definition is unique regardless of grid size and or the version of the
 * grid generator used.
 * 
 ...
 * identical grids in which only the cage id's are different result in identical
 * grid definitions.
 */
public class GridDefinitionCreator {
	...
	private String toStringPuzzleComplexityId() {
		return Integer.toString(gridGeneratingParameters
				.getPuzzleComplexity()
				.getId());
	}
	...
	private StringBuilder toStringCageIdPerCell() {
		...
		for (Cell cell : cells) {
			...
			// a cage-id > 99.
			cageIdPerCell.append(String.format("%02d",
					getNewCageId(cell.getCageId())));
		}
