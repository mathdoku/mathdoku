...
public class PuzzlePreferenceFragment extends AppPreferenceFragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (listPreference != null) {
			...
			listPreference.setValue(Preferences
					.getInstance()
		...
		if (preference != null) {
			preference
					.setSummary(getResources()
							.getString(
									Preferences.getInstance().getTheme() == Painter.GridTheme.LIGHT ? R.string.theme_light
											: R.string.theme_dark));
		}
	}
	...
	private void setInputMethodSummary() {
		...
		if (preference != null) {
			switch (Preferences.getInstance().getDigitInputMethod()) {
			case SWIPE_ONLY:
	...
	private void setOuterSwipeCircleSummary() {
		...
		if (preference != null) {
			if (Preferences.getInstance().isOuterSwipeCircleNeverVisible()) {
				preference
						.setSummary(getResources()
								.getString(
										R.string.puzzle_setting_outer_swipe_circle_visible_never));
			} else {
				String outerSwipeCircleVisibility = Preferences
						.getInstance()
						.getOuterSwipeCircleVisibility();
				preference
						.setSummary(getResources()
