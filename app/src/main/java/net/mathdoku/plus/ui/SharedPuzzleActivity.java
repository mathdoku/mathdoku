...
public class SharedPuzzleActivity extends AppFragmentActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Painter.getInstance()
                .setTheme(mMathDokuPreferences.getTheme());

        // set color
        findViewById(R.id.shared_puzzle_play_button).setBackgroundColor(mMathDokuPreferences.getTheme().getActionBarBackgroundColor());
    ...
    /**
     * Checks whether the specified intent is valid an can be processed by this activity.
     *
     * @param intent
     *         The intent to be checked.
     * @return True in case the intent contains a valid grid which can be processed.
     */
    private boolean isValidIntent(Intent intent) {
        ...
        String gridDefinition = new SharedPuzzle(this).getGridDefinitionFromUrl(uri);
        try {
            mGrid = new GridDefinition(gridDefinition).createGrid();
        } catch (InvalidGridException e) {
            Log.d("Cannot create a grid for definition '%s'.", gridDefinition, e);
            return false;
        }
    ...
    public void onClickPlayGame(@SuppressWarnings("UnusedParameters") View view) {
        ...
        GridRow gridRow = new GridDatabaseAdapter().getByGridDefinition(gridDefinition);
        if (gridRow == null) {
            startPuzzleFragment();
        } else {
    ...
    private void startPuzzleFragment() {
        ...
        if (mGrid.save()) {
            ...
            Intent intent = new Intent(this, PuzzleFragmentActivity.class).setFlags(
