...
public class PuzzleFragment extends android.support.v4.app.Fragment implements SharedPreferences
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mDigitPosition[2] = (Button) mRootView.findViewById(R.id.digitPosition3);
        mDigitPosition[3] = (Button) mRootView.findViewById(R.id.digitPosition4);
        mDigitPosition[4] = (Button) mRootView.findViewById(R.id.digitPosition5);
        mDigitPosition[5] = (Button) mRootView.findViewById(R.id.digitPosition6);
        mDigitPosition[6] = (Button) mRootView.findViewById(R.id.digitPosition7);
        mDigitPosition[7] = (Button) mRootView.findViewById(R.id.digitPosition8);
        mDigitPosition[8] = (Button) mRootView.findViewById(R.id.digitPosition9);

        mClearButton = (Button) mRootView.findViewById(R.id.clearButton);
        mClearButtonSwipeOnly = (Button) mRootView.findViewById(R.id.clearButtonSwipeOnly);

        mUndoButton = (Button) mRootView.findViewById(R.id.undoButton);
        mUndoButtonSwipeOnly = (Button) mRootView.findViewById(R.id.undoButtonSwipeOnly);
    ...
    public void onResume() {
        ...
        if (VERSION.SDK_INT >= VERSION_CODES.JELLY_BEAN_MR1) {
            ...
            // Register receiver
            mContext.registerReceiver(mDreamingBroadcastReceiver, intentFilter, null, null);
        }
     ...
     *
     * @return True in case the clear grid menu item is available. False otherwise.
     */
    ...
    void openClearGridDialog() {
        new AlertDialog.Builder(this.getActivity()).setTitle(R.string.dialog_clear_grid_confirmation_title)
                ...
                .setPositiveButton(R.string.dialog_clear_grid_confirmation_positive_button,
                                   new DialogInterface.OnClickListener() {
                                       @Override
    ...
    /**
     * Sets the handler on the grid which will be called as soon as the grid has been solved.
     */
    private void setOnSolvedHandler() {
        ...
        mGrid.setSolvedHandler(mGrid.new OnSolvedListener() {
            ...
            public void puzzleSolved() {
                ...
                if (!mGrid.isActive() || mGrid.isSolutionRevealed() || mGrid.countMoves() == 0) {
                    ...
                    // Build the animation
                    Animation animation = AnimationUtils.loadAnimation(mContext, R.anim.solved);
                    if (animation != null) {
    ...
    private void setNewGrid(Grid grid) {
        if (grid != null) {
            ...
            if (mGrid.isActive()) {
                ...
                setClearAndUndoButtonVisibility(mGrid == null ? null : mGrid.getSelectedCell());
                ...
                boolean copyModeEnabled = mMathDokuPreferences.isGridInputModeCopyEnabled();
                mGridPlayerView.setGridInputMode(mMathDokuPreferences.getGridInputMode(), false);
                if (copyModeEnabled) {
                    copyCellValues();
                }
            } else {
    ...
    void revealCell() {
        if (mGrid != null && mGrid.revealSelectedCell()) {
            ...
            CellRevealedCheat cellRevealedCheat = new CellRevealedCheat(mContext);
            if (mTimerTask != null) {
     ...
     *
     * @return True in case the reveal operator menu item is available. False otherwise.
     */
    ...
    void revealOperator() {
        if (mGrid != null && mGrid.revealOperatorSelectedCage()) {
            OperatorRevealedCheat operatorRevealedCheat = new OperatorRevealedCheat(mContext);
            if (mTimerTask != null) {
                mTimerTask.addCheatPenaltyTime(operatorRevealedCheat);
            }
            if (TipCheat.toBeDisplayed(mMathDokuPreferences, operatorRevealedCheat)) {
    ...
    boolean showCheckProgress() {
        return mGrid != null && mGrid.isActive() && !mGrid.containsNoEnteredValues();
    }
    ...
    void checkProgress() {
        ...
        int countNewInvalidChoices = allEnteredValuesValid ? 0 : mGrid.markInvalidChoices();
        ...
        if (TipCheat.toBeDisplayed(mMathDokuPreferences, checkProgressUsedCheat)) {
     ...
     *
     * @return True in case the reveal solution menu item is available. False otherwise.
     */
    ...
    void revealSolution() {
        ...
        new AlertDialog.Builder(this.getActivity()).setTitle(R.string.dialog_reveal_solution_confirmation_title)
                ...
                .setPositiveButton(R.string.dialog_reveal_solution_confirmation_positive_button,
                                   new DialogInterface.OnClickListener() {
                                       @Override
                                       public void onClick(DialogInterface dialog, int which) {
    ...
    @Override
    public void onInputModeChanged(final GridInputMode inputMode, boolean enableCopyMode) {
    ...
    private void setDigitPositionGrid() {
        ...
        if (mDigitPositionGrid == null || mDigitPositionGrid.isNotReusable(gridSize)) {
        ...
        for (int i = 0; i < mDigitPosition.length; i++) {
            ...
            mDigitPosition[i].setVisibility(mDigitPositionGrid.getVisibility(i));
        }

        // Visibility of third row of digits buttons
        if (mControlsPadBigTableRow3 != null) {
            mControlsPadBigTableRow3.setVisibility(gridSize >= 7 ? View.VISIBLE : View.GONE);
        }
    }
    ...
    private void setButtonLayout() {
        ...
        if (mControlsSwipeOnlyLinearLayout != null) {
            mControlsSwipeOnlyLinearLayout.setVisibility(swipeOnly ? View.VISIBLE : View.GONE);
     ...
     *
     * @return The resource id for the icon which matches the current input mode.
     */
     ...
     *
     * @return True in case the copy cell values menu item is available. False otherwise.
     */
    boolean showCopyCellValues() {
        if (mGrid == null || !mGrid.isActive() || mGridPlayerView.getGridInputMode() == GridInputMode.COPY) {
            return false;
        }
