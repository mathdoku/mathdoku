 ...
 */
// Suppress all warning because of string duplications. De-duplication of string makes this code
// less readable.
...
public class TestHelperCodeGenerator {
    ...
    private void appendGetHideOperator() {
        ...
        stringBuilder.append(String.format("return %s;\n",
                                           gridGeneratingParameters.isHideOperators() ? "true" :
                                                   "false"));
        stringBuilder.append("}\n");
        stringBuilder.append("\n");
    }
    ...
    private void appendPuzzleComplexity() {
        stringBuilder.append("protected PuzzleComplexity getPuzzleComplexity() {\n");
    ...
    private void appendGetCorrectValuePerCell() {
        ...
        for (int row = 0; row < grid.getGridSize(); row++) {
            ...
            for (int col = 0; col < grid.getGridSize(); col++) {
                stringBuilder.append(String.format("%d, ", grid.getCellAt(row, col)
                        .getCorrectValue()));
            }
            stringBuilder.append("\n");
        }
    ...
    private void appendGetCageOperatorPerCage() {
        stringBuilder.append("protected CageOperator[] getCageOperatorPerCage() {\n");
    ...
    private void appendGetGridDefinition() {
        ...
        for (int row = 0; row < grid.getGridSize(); row++) {
            stringBuilder.append(String.format("// Cage ids for cells on row %d\n", row));
            stringBuilder.append(".append(\"");
            for (int col = 0; col < grid.getGridSize(); col++) {
                stringBuilder.append(String.format("%02d", grid.getCellAt(row, col)
                        .getCageId()));
            }
            stringBuilder.append("\")\n");
        }
        ...
        for (Cage cage : grid.getCages()) {
            stringBuilder.append(String.format("// Definition for cage id %d\n", cage.getId()));
            stringBuilder.append(".append(FIELD_SEPARATOR_GRID_DEFINITION_PART)\n");
            stringBuilder.append(
                    String.format(".append(\"%d,%d,%d\")\n", cage.getId(), cage.getResult(),
                                  getCageOperator(cage).getId()));
        }
    ...
    private CageOperator getCageOperator(Cage cage) {
        return gridGeneratingParameters.isHideOperators() ? CageOperator.NONE : cage.getOperator();
    }
