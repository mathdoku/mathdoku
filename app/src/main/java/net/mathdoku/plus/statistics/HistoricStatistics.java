...
public class HistoricStatistics {
    ...
    private class SeriesSummary {
        ...
        public void addDataPoint(HistoricStatisticsSelector.DataPoint dataPoint) {
            long totalValue = dataPoint.getElapsedTimeExcludingCheatPenalty() + dataPoint
                    .getCheatPenalty();
    ...
    /**
     * Creates a new instance of {@link HistoricStatistics}. Note that order of columns in cursors
     * is defined.
     *
     * @param mMinGridSize
     *         The minimum grid size for which the statistics have to be displayed.
     * @param mMaxGridSize
     *         The maximum grid size for which the statistics have to be displayed.
     */
    public HistoricStatistics(int mMinGridSize, int mMaxGridSize) {
        dataPoints = new HistoricStatisticsSelector(mMinGridSize, mMaxGridSize).getDataPointList();
    ...
    /**
     * Checks whether at least one data point for the given status exists. If so, the corresponding
     * series can be plotted.
     *
     * @param solvingAttemptStatus
     *         The required status of the data points to be selected. Use null in case no selection
     *         on status has to be made.
        ...
        for (HistoricStatisticsSelector.DataPoint dataPoint : dataPoints) {
            if (index >= start) {
                if (dataPoint.getSolvingAttemptStatus() == solvingAttemptStatus ||
                    ...
                    if (includeElapsedTime && dataPoint.getElapsedTimeExcludingCheatPenalty() > 0
    ...
    /**
     * Converts the given series to a XYSeries object which can be processed by AChartEngine.
     *
     * @param solvingAttemptStatus
     *         The required status ({@value net.mathdoku.plus.enums.SolvingAttemptStatus
     *         .UNFINISHED} or {@value net.mathdoku.plus.enums.SolvingAttemptStatus
     ...
     */
    @SuppressWarnings("SameParameterValue")
    public XYSeries getXYSeries(SolvingAttemptStatus solvingAttemptStatus, String title,
                                Scale scale, boolean includeElapsedTime, boolean includeCheatTime) {
            ...
            if (index >= start) {
                ...
                if (dataPoint.getSolvingAttemptStatus() == solvingAttemptStatus) {
                    ...
                    value = (includeElapsedTime ? dataPoint.getElapsedTimeExcludingCheatPenalty()
                            : 0) + (includeCheatTime ? dataPoint.getCheatPenalty() : 0);
    ...
    /**
     * Converts the solution revealed series to a XYSeries object which can be processed by
     * AChartEngine.
     *
     * @param title
     *         The title to be used in the XYSeries.
                ...
                if (dataPoint.getSolvingAttemptStatus() == SolvingAttemptStatus.REVEALED_SOLUTION) {
                    if (includeElapsedTime) {
                        value = includeCheatTime ? maxY : Math.min(
                                dataPoint.getElapsedTimeExcludingCheatPenalty(), maxY);
                    } else {
                        value = Math.max(
                                Math.min(maxY - dataPoint.getElapsedTimeExcludingCheatPenalty(),
     ...
     */
    @SuppressWarnings("SameParameterValue")
    public XYSeries getXYSeriesHistoricAverage(SolvingAttemptStatus solvingAttemptStatus,
                                               String title, Scale scale) {
        XYSeries xySeries = new XYSeries(title);
        ...
        for (HistoricStatisticsSelector.DataPoint dataPoint : dataPoints) {
            if (solvingAttemptStatus == null || dataPoint.getSolvingAttemptStatus() ==
    ...
    /**
     * Gets the maximum Y values to be used for series. Y-values of grid in which the solution was
     * revealed will not be taken into account unless no grids have been solved or unfinished.
     *
     * @return The maximum Y-value to be used for displaying the historic statistics.
     */
    public double getMaxY(Scale scale) {
        double scaleFactor = getScaleFactor(scale);

        if (mSolvedSeriesSummary.getCount() > 0 || mUnfinishedSeriesSummary.getCount() > 0) {
    ...
    /**
     * Converts a given scale to a scaleFactor which can be applied on the value in the data
     * points.
     *
     * @param scale
