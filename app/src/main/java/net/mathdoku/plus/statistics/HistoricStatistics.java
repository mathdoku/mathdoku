...
public class HistoricStatistics {
    ...
    private class SeriesSummary {
        ...
        public void addDataPoint(HistoricStatisticsSelector.DataPoint dataPoint) {
            long totalValue = dataPoint.getElapsedTimeExcludingCheatPenalty() + dataPoint.getCheatPenalty();
    ...
    /**
     * Creates a new instance of {@link HistoricStatistics}. Note that order of columns in cursors is defined.
     *
     * @param mMinGridSize
     ...
     *
     * @param solvingAttemptStatus
     *         The required status ({@value net.mathdoku.plus.enums.SolvingAttemptStatus .UNFINISHED} or {@value
     *         net.mathdoku.plus.enums.SolvingAttemptStatus .FINISHED_SOLVED}) of the data points to be selected.
     * @param title
    ...
    /**
     * Converts the solution revealed series to a XYSeries object which can be processed by AChartEngine.
     *
     * @param title
     *         The title to be used in the XYSeries.
     * @param maxY
     ...
     */
    @SuppressWarnings("SameParameterValue")
    public XYSeries getXYSeriesHistoricAverage(SolvingAttemptStatus solvingAttemptStatus, String title, Scale scale) {
        XYSeries xySeries = new XYSeries(title);
        ...
        for (HistoricStatisticsSelector.DataPoint dataPoint : dataPoints) {
            if (solvingAttemptStatus == null || dataPoint.getSolvingAttemptStatus() == solvingAttemptStatus) {
    ...
    /**
     * Gets the maximum Y values to be used for series. Y-values of grid in which the solution was revealed will not be
     * taken into account unless no grids have been solved or unfinished.
     *
    ...
    public double getMaxY(Scale scale) {
        ...
        if (mSolvedSeriesSummary.getCount() > 0 || mUnfinishedSeriesSummary.getCount() > 0) {
            return Math.max(mSolvedSeriesSummary.getMaximum(), mUnfinishedSeriesSummary.getMaximum()) / scaleFactor;
        } else {
    ...
    /**
     * Converts a given scale to a scaleFactor which can be applied on the value in the data points.
     *
     * @param scale
