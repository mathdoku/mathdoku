...
public class GridSaverTest {
    ...
    public void setup() {
        ...
        long dateCreated = new java.util.GregorianCalendar(2014, Calendar.FEBRUARY, 8, 7,
                                                           6).getTimeInMillis(); // 2014, february
        // 8th at 07:06
    ...
    @Test
    public void save_InsertGridWithUniqueGridDefinition_Saved() throws Exception {
        // Prepare for not finding an existing grid row with the same definition
        when(mGridDatabaseAdapterMock.getByGridDefinition(anyString())).thenReturn(null);
        // Prepare for insert of new grid row
        int mGridRowIdAfterSuccessfulInsert = 34;
        when(mGridDatabaseAdapterMock.insert(any(Grid.class))).thenReturn(mGridRowIdAfterSuccessfulInsert);
        // Prepare insert of new solving attempt
        when(mSolvingAttemptDatabaseAdapterMock.insert(any(SolvingAttemptRow.class))).thenReturn(
                mock(SolvingAttemptRow.class));
        // Prepare insert of new statistics
        int mGridStatisticsIdAfterSuccessfulInsert = 52;
        when(mStatisticsDatabaseAdapterMock.insert(any(GridStatistics.class))).thenReturn(
                mGridStatisticsIdAfterSuccessfulInsert);
        ...
        verify(mSolvingAttemptDatabaseAdapterMock).insert(any(SolvingAttemptRow.class));
    ...
    @Test
    public void save_InsertGridWithUniqueGridDefinitionButInsertGridFails_NotSaved() throws Exception {
        // Prepare for not finding an existing grid row with the same definition
        when(mGridDatabaseAdapterMock.getByGridDefinition(anyString())).thenReturn(null);
        // Prepare for failing insert of new grid row
        int mGridRowIdWhenInsertFails = -1;
        when(mGridDatabaseAdapterMock.insert(any(Grid.class))).thenReturn(mGridRowIdWhenInsertFails);
    ...
    @Test
    public void save_InsertGridWithExistingGridDefinition_Saved() throws Exception {
        ...
        when(mGridDatabaseAdapterMock.getByGridDefinition(anyString())).thenReturn(gridRow);
        // Prepare insert of new solving attempt
        when(mSolvingAttemptDatabaseAdapterMock.insert(any(SolvingAttemptRow.class))).thenReturn(
                mock(SolvingAttemptRow.class));
        // Prepare insert of new statistics
    ...
    @Test
    public void save_SavedGridButMissingSolvingAttemptAndStatistics_Saved() throws Exception {
    ...
    @Test
    public void save_SavedGridButInsertSolvingAttemptFails_NotSaved() throws Exception {
        // Grid was already saved
        int existingGridId = 12;
        when(mGridMock.getRowId()).thenReturn(existingGridId);
        // Prepare for failing insert of new solving attempt
        when(mSolvingAttemptDatabaseAdapterMock.insert(any(SolvingAttemptRow.class))).thenReturn(null);
        boolean saveResult = mGridSaver.save(mGridMock);
    ...
    @Test
    public void save_ExistingGridAndSolvingAttemptUpdateSolvingAttemptAndInsertStatistics_Saved() throws Exception {
        // Grid and solving attempt were already saved
        int existingGridId = 12;
        when(mGridMock.getRowId()).thenReturn(existingGridId);
        int existingSolvingAttemptId = 15;
        when(mGridMock.getSolvingAttemptId()).thenReturn(existingSolvingAttemptId);

        // Prepare successful update of solving attempt
        when(mSolvingAttemptDatabaseAdapterMock.update(any(SolvingAttemptRow.class))).thenReturn(true);
        // Prepare insert of new statistics
        int mGridStatisticsIdAfterSuccessfulInsert = 52;
        when(mStatisticsDatabaseAdapterMock.insert(any(GridStatistics.class))).thenReturn(
                mGridStatisticsIdAfterSuccessfulInsert);
        ...
        verify(mDatabaseHelperMock).beginTransaction();
        verify(mGridDatabaseAdapterMock, never()).insert(any(Grid.class));
        verify(mSolvingAttemptDatabaseAdapterMock, never()).insert(any(SolvingAttemptRow.class));
        verify(mSolvingAttemptDatabaseAdapterMock).update(any(SolvingAttemptRow.class));
        verify(mStatisticsDatabaseAdapterMock).insert(any(GridStatistics.class));
        verify(mDatabaseHelperMock).setTransactionSuccessful();
        verify(mDatabaseHelperMock).endTransaction();
        assertThat(mGridSaver.getRowId(), is(existingGridId));
        assertThat(mGridSaver.getSolvingAttemptId(), is(existingSolvingAttemptId));
        assertThat(mGridSaver.getGridStatistics().mId >= 0, is(true));
    ...
    @Test
    public void save_ExistingGridAndSolvingAttemptButUpdateSolvingAttemptFails_NotSaved() throws Exception {
        // Grid and solving attempt were already saved
        int existingGridId = 12;
        when(mGridMock.getRowId()).thenReturn(existingGridId);
        int existingSolvingAttemptId = 15;
        ...
        when(mSolvingAttemptDatabaseAdapterMock.update(any(SolvingAttemptRow.class))).thenReturn(false);
    ...
    @Test
    public void save_ExistingGridAndSolvingAttemptButInsertStatisticsFails_NotSaved() throws Exception {
        // Grid and solving attempt were already saved
        int existingGridId = 12;
        when(mGridMock.getRowId()).thenReturn(existingGridId);
        int existingSolvingAttemptId = 15;
        ...
        when(mStatisticsDatabaseAdapterMock.insert(any(GridStatistics.class))).thenReturn(-1);
    ...
    @Test
    public void save_ExistingGridAndSolvingAttemptAndStatisticsUpdateStatisticsFails_NotSaved() throws Exception {
        // Grid and solving attempt were already saved
        int existingGridId = 12;
        when(mGridMock.getRowId()).thenReturn(existingGridId);
        int existingSolvingAttemptId = 15;
    ...
    @Test
    public void save_ExistingGridAndSolvingAttemptAndStatisticsUpdateAfterReplayIsFinished_Saved() throws Exception {
        // Grid and solving attempt were already saved
        int existingGridId = 12;
        when(mGridMock.getRowId()).thenReturn(existingGridId);
        int existingSolvingAttemptId = 15;
        ...
        verify(mStatisticsDatabaseAdapterMock, never()).insert(any(GridStatistics.class));
        verify(mGridStatisticsMock).save();
        verify(mStatisticsDatabaseAdapterMock).updateSolvingAttemptToBeIncludedInStatistics(anyInt(), anyInt());
        verify(mDatabaseHelperMock).setTransactionSuccessful();
        verify(mDatabaseHelperMock).endTransaction();
        assertThat(mGridSaver.getRowId(), is(existingGridId));
        assertThat(mGridSaver.getSolvingAttemptId(), is(existingSolvingAttemptId));
    ...
    public void getRowId_MethodIsCalledBeforeCallingSave_ThrowUnexpectedMethodInvocationException() throws Exception {
        mGridSaver.getRowId();
    }

    @Test(expected = UnexpectedMethodInvocationException.class)
    public void getSolvingAttemptId_MethodIsCalledBeforeCallingSave_ThrowUnexpectedMethodInvocationException() throws Exception {
        mGridSaver.getSolvingAttemptId();
    }

    @Test(expected = UnexpectedMethodInvocationException.class)
    public void getGridStatistics_MethodIsCalledBeforeCallingSave_ThrowUnexpectedMethodInvocationException() throws Exception {
