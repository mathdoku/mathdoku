...
public class OverlappingSubsetCheckerTest {
	...
	@Test
	public void hasOverlap_NewCageHasOverlappingRowWithOtherCage_OverlapDetected()
			throws Exception {
	...
	@Test
	public void hasOverlap_NewCageHasOverlappingColumnWithOtherCage_OverlapDetected()
			throws Exception {
		Boolean[] usedForNewCage = new Boolean[/* cell id */] {
				// row 1
				false, false, false, false,
	...
	@Test
	public void hasOverlap_NewCageHasNoOverlappingSubsetWithOtherCage_NoOverlapDetected()
			throws Exception {
		Boolean[] usedForNewCage = new Boolean[/* cell id */] {
				// row 1
				false, false, false, false,
	...
	private void assertOverlap(Integer[] correctValuesPerCell,
			Integer[] cageIds, Boolean[] usedForNewCage,
			Matcher<Boolean> overlapMatcher) {
		int size = 4;
		assertThat(size * size, is(correctValuesPerCell.length));

		Matrix<Integer> correctValuesMatrix = toIntegerMatrix(
				correctValuesPerCell, size);
		Matrix<Integer> cageIdMatrix = toIntegerMatrix(cageIds, size);
		Matrix<Boolean> newCageMatrix = toBooleanMatrix(usedForNewCage, 4);
		assertThat(
				new OverlappingSubsetChecker(correctValuesMatrix).hasOverlap(
						cageIdMatrix, newCageMatrix), overlapMatcher);
	}
