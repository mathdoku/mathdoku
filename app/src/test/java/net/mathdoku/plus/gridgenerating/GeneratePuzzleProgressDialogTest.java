...
public class GeneratePuzzleProgressDialogTest {
	...
	private class GridGeneratorAsyncTaskStub implements
			GridGeneratorAsyncTaskIface {
	...
	private GeneratePuzzleProgressDialog createGeneratePuzzleProgressDialog(
			final GridGeneratingParameters[] gridGeneratingParametersArray) {
		return new GeneratePuzzleProgressDialog(puzzleFragmentActivity,
	...
	public void constructor_Default_SpinnerStyle() throws Exception {
		verify(progressDialogMock).setProgressStyle(
				ProgressDialog.STYLE_SPINNER);
	}

	@Test
	public void constructor_MultipleGridGeneratingParameterSets_ProgressBarStyle()
			throws Exception {
	...
	private GeneratePuzzleProgressDialog createGeneratePuzzleProgressDialogForMultipleGrids() {
		return createGeneratePuzzleProgressDialog(new GridGeneratingParameters[] {
				mock(GridGeneratingParameters.class),
				mock(GridGeneratingParameters.class) });
	}

	@Test
	public void attachToActivity_AttachActivityAgainWhileDialogStillVisible_SameInstanceReturned()
			throws Exception {
	...
	@Test
	public void attachToActivity_AttachActivityAgainWhileDialogNotVisible_OtherInstanceReturned()
			throws Exception {
		...
		assertThat(
				generatePuzzleProgressDialog
						.attachToActivity(puzzleFragmentActivity),
				is(sameInstance(generatePuzzleProgressDialog)));
		assertThat(progressDialogMock,
				is(notSameInstance(initialProgressDialog)));
	}

	@Test
	public void attachToActivity_AttachOtherActivity_OtherInstanceReturned()
			throws Exception {
		// In setup the progress dialog is already attached to the puzzle
		// fragment and a dialog was created. After re-attaching the same
		// activity, the dialog should not have been rebuild.
		...
		assertThat(
				generatePuzzleProgressDialog
						.attachToActivity(otherPuzzleFragmentActivity),
				is(sameInstance(generatePuzzleProgressDialog)));
		assertThat(progressDialogMock,
				is(notSameInstance(initialProgressDialog)));
	}
	...
	@Test
	public void onFinishGridGenerator_GridListNull_DismissDialogOnly()
			throws Exception {
	...
	@Test
	public void onFinishGridGenerator_GridListEmpty_DismissDialogOnly()
			throws Exception {
		generatePuzzleProgressDialog
				.onFinishGridGenerator(new ArrayList<Grid>());
		verify(progressDialogMock).dismiss();
	}

	@Test
	public void onFinishGridGenerator_GridListContainsSingleGrid_DismissDialogOnly()
			throws Exception {
		List<Grid> gridListWithSingleGrid = new ArrayList<Grid>();
		gridListWithSingleGrid.add(mock(Grid.class));
		generatePuzzleProgressDialog
				.onFinishGridGenerator(gridListWithSingleGrid);
		verify(progressDialogMock).dismiss();
