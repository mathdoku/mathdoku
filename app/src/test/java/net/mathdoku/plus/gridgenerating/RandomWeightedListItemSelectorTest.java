...
public class RandomWeightedListItemSelectorTest {
	...
	private List<WeightedItem> getWeightedListWithNumberOfItems(
			int numberOfItems) {
		List<WeightedItem> listWeightedItems = new ArrayList<WeightedItem>();
		for (int i = 1; i <= Math.min(numberOfItems, stringItems.length); i++) {
			WeightedItem<String> weightedItem = new WeightedItem<String>(
					stringItems[i - 1], weightItems[i - 1]);
	...
	public void constructor_NullRandomizer_IllegalArgumentException()
			throws Exception {
		new RandomWeightedListItemSelector(null,
				getWeightedListWithNumberOfItems(1));
	}

	@Test(expected = IllegalArgumentException.class)
	public void constructor_NullWeightedList_IllegalArgumentException()
			throws Exception {
		new RandomWeightedListItemSelector(new Random(), null);
	}

	@Test(expected = IllegalArgumentException.class)
	public void constructor_EmptyWeightedList_IllegalArgumentException()
			throws Exception {
		new RandomWeightedListItemSelector(new Random(),
				getWeightedListWithNumberOfItems(0));
	}

	@Test
	public void constructor_WeightedListWithOneItem_IllegalArgumentException()
			throws Exception {
		new RandomWeightedListItemSelector(new Random(),
				getWeightedListWithNumberOfItems(1));
	}
	...
	@Test
	public void constructor_WeightedListWithMultipleItems_IllegalArgumentException()
			throws Exception {
		new RandomWeightedListItemSelector(new Random(),
				getWeightedListWithNumberOfItems(2));
	}

	@Test
	public void setLastItemToBeSelected_ListWithOneItemWhichIsSetAsLastItem_ItemCorrectlyReturned()
			throws Exception {
		randomWeightedListItemSelector = new RandomWeightedListItemSelector(
				new Random(), getWeightedListWithNumberOfItems(1));
		int indexToBeSelectedAsLastItem = 0;
		randomWeightedListItemSelector
				.setLastItemToBeSelected(stringItems[indexToBeSelectedAsLastItem]);
		assertThat((String) randomWeightedListItemSelector.next(),
				is(stringItems[indexToBeSelectedAsLastItem]));
	}

	@Test
	public void setLastItemToBeSelected_ListWithMultipleItemsFromWhichOneItemIsSetAsLastItem_ItemCorrectlyReturnedAsLastItem()
			throws Exception {
		randomWeightedListItemSelector = new RandomWeightedListItemSelector(
				new Random(),
				getWeightedListWithNumberOfItems(stringItems.length));
	...
	@Test
	public void setLastItemToBeSelected_ListWithMultipleItemsAndLastItemNotInOriginalList_ItemCorrectlyReturnedAsLastItem()
			throws Exception {
		randomWeightedListItemSelector = new RandomWeightedListItemSelector(
				new Random(),
				getWeightedListWithNumberOfItems(stringItems.length));
	...
	public void next_TooManyItemRetrievedFromList_IllegalStateExceptionThrown()
			throws Exception {
		randomWeightedListItemSelector = new RandomWeightedListItemSelector(
				new Random(), getWeightedListWithNumberOfItems(1));
		assertThat((String) randomWeightedListItemSelector.next(),
	...
	@Test
	public void next_RetrieveSpecificItemByWeight_ExpectedItemReturned()
			throws Exception {
	...
	private void assertThatWeightResultsInItem(int weight, int indexExpectedItem) {
		...
		randomWeightedListItemSelector = new RandomWeightedListItemSelector(
				...
				getWeightedListWithNumberOfItems(stringItems.length));
		assertThat((String) randomWeightedListItemSelector.next(),
				is(stringItems[indexExpectedItem]));
	}

	@Test
	public void next_RetrieveItemsInOrderSpecifiedByRandomized_ItemsReturnedInCorrectOrder()
			throws Exception {
		Random randomMock = mock(Random.class);
		...
		when(randomMock.nextInt(anyInt())).thenReturn(randomWeightsInOrder[0],
				randomWeightsInOrder[1], randomWeightsInOrder[2],
				randomWeightsInOrder[3], randomWeightsInOrder[4]);
		...
		for (int i = 0; i < resultItemsInOrder.length; i++) {
			resultItemsInOrder[i] = String
					.valueOf(randomWeightedListItemSelector.next());
		}
