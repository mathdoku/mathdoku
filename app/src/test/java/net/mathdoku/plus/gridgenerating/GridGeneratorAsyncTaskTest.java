...
public class GridGeneratorAsyncTaskTest {
    ...
    public void setUp() throws Exception {
        ...
        gridGeneratorAsyncTask = new GridGeneratorAsyncTask(gridGeneratorAsyncTaskListener,
                                                            new GridGeneratorAsyncTask.ObjectsCreator() {
                                                                @Override
                                                                public GridGeneratorIface createGridGenerator
                                                                        (GridGeneratorAsyncTask
                                                                                 gridGeneratorAsyncTask) {
                                                                    return new GridGeneratorStub(
                                                                            gridGeneratorAsyncTask);
                                                                }
                                                            });
    }
    ...
    public void constructor_NullListener_ThrowsIllegalArgumentException() throws Exception {
        new GridGeneratorAsyncTask(null);
    }

    // AsyncTask rethrows exceptions always as runtime exception.
    @Test(expected = RuntimeException.class)
    public void execute_NoGridGeneratingParameters_ThrowsRuntimeException() throws Exception {
        gridGeneratorAsyncTask.execute();
    }

    // AsyncTask rethrows exceptions always as runtime exception.
    @Test(expected = RuntimeException.class)
    public void execute_EmptyGridGeneratingParametersArray_ThrowsGridGeneratingException() throws Exception {
    ...
    @Test
    public void updateProgressHighLevel_AnyText_AsyncTaskListenerIsCalled() throws Exception {
        final String HIGH_LEVEL_PROGRESS_UPDATE = "*** HIGH LEVEL PROGRESS UPDATE ***";
        gridGeneratorAsyncTask.updateProgressHighLevel(HIGH_LEVEL_PROGRESS_UPDATE);
        assertThat(gridGeneratorAsyncTaskListener.onHighLevelProgressUpdateText, is(HIGH_LEVEL_PROGRESS_UPDATE));
    }

    @Test
    public void updateProgressDetailLevel_AnyText_AsyncTaskListenerIsCalled() throws Exception {
        final String DETAILS_LEVEL_PROGRESS_UPDATE = "*** DETAILS LEVEL PROGRESS UPDATE ***";
        gridGeneratorAsyncTask.updateProgressDetailLevel(DETAILS_LEVEL_PROGRESS_UPDATE);
        assertThat(gridGeneratorAsyncTaskListener.onDetailLevelProgressUpdateText, is(DETAILS_LEVEL_PROGRESS_UPDATE));
    }

    @Test
    public void signalSlowGridGeneration_OnAnyCall_AsyncTaskIsCancelled() throws Exception {
    ...
    @Test
    public void onPostExecute_OneSetOfGridGeneratingParameters_TaskIsExecuted() throws Exception {
        gridGeneratorAsyncTask.execute(mock(GridGeneratingParameters.class));
        assertThat(gridGeneratorAsyncTaskListener.onFinishGridGeneratorIsCalled, is(true));
        assertThat(gridGeneratorAsyncTaskListener.onFinishGridGeneratorGridCount, is(1));
    }

    @Test
    public void onPostExecute_TwoSetsOfGridGeneratingParameters_TaskIsExecuted() throws Exception {
        if (Config.APP_MODE == Config.AppMode.DEVELOPMENT) {
            gridGeneratorAsyncTask.execute(mock(GridGeneratingParameters.class), mock(GridGeneratingParameters.class));
            assertThat(gridGeneratorAsyncTaskListener.onFinishGridGeneratorIsCalled, is(true));
            assertThat(gridGeneratorAsyncTaskListener.onFinishGridGeneratorGridCount, is(2));
        }
    }
    ...
    public void testOnCancelled() throws Exception {
        ...
        assertThat(gridGeneratorAsyncTaskListener.onCancelGridGeneratorAsyncTaskIsCalled, is(true));
    }
}
