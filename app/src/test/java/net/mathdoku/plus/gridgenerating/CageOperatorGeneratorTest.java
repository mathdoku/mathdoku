...
public class CageOperatorGeneratorTest {
    ...
    private class Setup {
        ...
        CageOperator[] orderedCageOperatorsInGenerator = new CageOperator[]{CageOperator.NONE, CageOperator.DIVIDE,
                CageOperator.SUBTRACT, CageOperator.MULTIPLY, CageOperator.ADD};
        int[] weightsForOrderedCageOperatorsInGenerator = new int[]{100, 50, 30, 15, 15};

        public Setup() {
            cellValues = TWO_CELL_VALUES;
            cageOperators = new CageOperator[]{CageOperator.ADD, CageOperator.SUBTRACT, CageOperator.MULTIPLY,
                    CageOperator.DIVIDE};
        }
        ...
        public CageOperatorGenerator createCageOperatorGeneratorWhichReturns(CageOperator targetCageOperator) {
            ...
            when(randomMock.nextInt(anyInt())).thenReturn(
        ...
        private int getRandomWhichResultsInSelectingCageOperator(CageOperator... cageOperator) {
            int index = Arrays.asList(orderedCageOperatorsInGenerator)
                    .indexOf(cageOperator);
            int random = 0;
    ...
    public void getCageOperator_1Value_Specified_NoneCageOperator() throws Exception {
        Setup setup = new Setup().setCellValues(ONE_CELL_VALUE)
