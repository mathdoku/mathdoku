...
public class GridDatabaseAdapterTest {
	...
	public void getDatabaseTableDefinition() throws Exception {
		String resultDatabaseCreateSQL = new GridDatabaseAdapter()
		...
		expectedDatabaseCreateSQL
				.append("`generator_revision_number` integer, ");
	...
	public void insert() throws Exception {
		...
		assertThat(
				gridDatabaseAdapter.insert(GridCreator4x4.createEmptyGrid()),
				is(idOfFirstGridInEmptyDatabase));
	}
	...
	public void get() throws Exception {
		...
		assertThat(gridDatabaseAdapter.get(gridId),
				is(getExpectedGridRowFromHashMap(gridId)));
	}
	...
	private int insertGridAndAddToHashMap(Grid grid) {
		...
		GridRow expectedGridRow = new GridRow(gridId, grid.getDefinition(),
				grid.getGridSize(), grid.getDateCreated(),
	...
	@Test
	public void getByGridDefinition() throws Exception {
		insertGridAndAddToHashMap(GridCreator4x4.createEmptyGrid());
		int gridId = insertGridAndAddToHashMap(GridCreator2x2.createEmptyGrid());
		insertGridAndAddToHashMap(GridCreator5x5.createEmptyGrid());

		String gridDefinition = getExpectedGridRowFromHashMap(gridId)
				.getGridDefinition();
		assertThat(gridDatabaseAdapter.getByGridDefinition(gridDefinition),
				is(getExpectedGridRowFromHashMap(gridId)));
	}

	@Test
	public void getPrefixedColumnName() throws Exception {
		assertThat(GridDatabaseAdapter.getPrefixedColumnName("TestAbC"),
				is("`grid`.`TestAbC`"));
	}
}
