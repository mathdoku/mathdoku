...
public class HistoricStatisticsSelectorTest {
    ...
    public void setUp() throws Exception {
        if (!setupAtFirstTestIsExecuted) {
            ...
            new DatabaseCsvImporter(pathToImportFile + "HistoricStatisticsSelectorTest_statistics.csv",
    ...
    public void invoke_GetAllGridsWithSize4() throws Exception {
        int minGridSize = 4;
        int maxGridSize = 4;
        assertThat(new HistoricStatisticsSelector(minGridSize, maxGridSize).getDataPointList(),
                   is(getDataPoints(minGridSize, maxGridSize)));
    }

    private List<HistoricStatisticsSelector.DataPoint> getDataPoints(int minGridSize, int maxGridSize) {
        List<HistoricStatisticsSelector.DataPoint> expectedDataPoints = new ArrayList<HistoricStatisticsSelector
                .DataPoint>();
        if (minGridSize <= 4) {
            expectedDataPoints.add(createDataPoint(78512, 40000, SolvingAttemptStatus.FINISHED_SOLVED));
            expectedDataPoints.add(createDataPoint(128038, 100000, SolvingAttemptStatus.UNFINISHED));
        }
        if (maxGridSize >= 5) {
            expectedDataPoints.add(createDataPoint(153219, 80000, SolvingAttemptStatus.REVEALED_SOLUTION));
    ...
    private HistoricStatisticsSelector.DataPoint createDataPoint(long elapsedTimeExcludingCheatPenalty,
                                                                 long cheatPenalty,
        ...
        HistoricStatisticsSelector.DataPoint dataPoint = new HistoricStatisticsSelector.DataPoint(
                elapsedTimeExcludingCheatPenalty - cheatPenalty, cheatPenalty, solvingAttemptStatus);
        return dataPoint;
    }
