...
public class DatabaseAdapterTest {
    ...
    private class DatabaseAdapterStub extends DatabaseAdapter {
        ...
        public DatabaseTableDefinition getDatabaseTableDefinition() {
            ...
            databaseTableDefinition.addColumn(new DatabaseColumnDefinition("column", DataType.STRING));
            if (hasAdditionalColumn) {
                databaseTableDefinition.addColumn(new DatabaseColumnDefinition("additional_column", DataType.STRING));
            }
    ...
    public void getAllDatabaseAdapters() throws Exception {
        DatabaseAdapter[] databaseAdapters = DatabaseAdapterStub.getAllDatabaseAdapters(sqLiteDatabase);

        List<String> databaseAdapterClassNames = new ArrayList<String>();
        for (DatabaseAdapter databaseAdapter : databaseAdapters) {
            databaseAdapterClassNames.add(databaseAdapter.getClass()
                                                  .getSimpleName());
        }
        Collections.sort(databaseAdapterClassNames);

        List<String> expectedDatabaseAdapterClassNames = Arrays.asList(
                new String[]{GridDatabaseAdapter.class.getSimpleName(),
        ...
        assertThat(databaseAdapterClassNames, is(equalTo(expectedDatabaseAdapterClassNames)));
    }

    @Test
    public void createTable_TableDoesNotYetExist_TableIsCreated() throws Exception {
        assertThat(databaseAdapterStub.createTable(), is(true));
    }

    @Test(expected = DatabaseAdapterException.class)
    public void createTable_TableAlreadyExists_ThrowException() throws Exception {
    ...
    public void getCurrentDefinitionOfDatabaseTable() throws Exception {
        assertThat(databaseAdapterStub.getCurrentDefinitionOfDatabaseTable(), is(""));
        databaseAdapterStub.createTable();
        assertThat(databaseAdapterStub.getCurrentDefinitionOfDatabaseTable()
                           .toUpperCase(), is(databaseTableDefinition.getCreateTableSQL()
                                                      .toUpperCase()));
    }

    @Test
    public void isExistingDatabaseTable_TableDoesNotYetExist_False() throws Exception {
    ...
    @Test
    public void isTableDefinitionChanged_TableExistsAndIsNotChanged_False() throws Exception {
        databaseAdapterStub.createTable();
        assertThat(databaseAdapterStub.isTableDefinitionChanged(), is(false));
    }

    @Test
    public void isTableDefinitionChanged_TableExistsAndIsChanged_True() throws Exception {
        assertThat(databaseAdapterStub.createTable(), is(true));
        ...
        assertThat(databaseAdapterStubWithAdditionalColumn.isTableDefinitionChanged(), is(true));
    }
    ...
    public void recreateTableInDevelopmentMode() throws Exception {
        if (Config.APP_MODE == Config.AppMode.DEVELOPMENT) {
            ...
            databaseAdapterStubWithAdditionalColumn.recreateTableInDevelopmentMode();

            assertThat(databaseAdapterStubWithAdditionalColumn.isTableDefinitionChanged(), is(false));
        }
    }
