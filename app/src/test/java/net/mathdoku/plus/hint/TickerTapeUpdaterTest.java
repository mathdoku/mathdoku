...
public class TickerTapeUpdaterTest {
	...
	@Before
	public void setUp() {
		tickerTapeUpdater = new TickerTapeUpdater(
				tickerTapeMock);
	}

	@Test
	public void startMoving_InitialOrReset_TickerTapeIsVisible()
			throws Exception {
	...
	@Test
	public void run_MultipleTimesBeforeEraseConditionIsFulfilled_TickerTapeHidden()
			throws Exception {
		boolean doNotCancelBeforeEraseConditionIsFulfilled = false;
		runMultipleTimes(doNotCancelBeforeEraseConditionIsFulfilled);

		verify(tickerTapeMock).setVisibility(View.VISIBLE);
		verify(tickerTapeMock,
				times(NUMBER_OF_RUNS_BEFORE_ERASE_CONDITION_FULFILLED))
				.isEraseConditionFulfilled(anyInt(), anyLong());
	...
	private void runMultipleTimes(boolean cancelBeforeEraseConditionIsFulfilled) {
		...
		// returns true.
		when(tickerTapeMock.isEraseConditionFulfilled(anyInt(), anyLong()))
	...
	@Test
	public void run_MultipleTimeBeforeCancellation_TickerTapeNotHidden()
			throws Exception {
		boolean cancelBeforeEraseConditionIsFulfilled = true;
		runMultipleTimes(cancelBeforeEraseConditionIsFulfilled);

		verify(tickerTapeMock).setVisibility(View.VISIBLE);
		verify(tickerTapeMock, times(getNumberOfRunsBeforeCancellation()))
				.isEraseConditionFulfilled(anyInt(), anyLong());
