...
public class ArchiveFragmentGridTypeFilterSpinnerTest {
    ...
    public void setup() {
        ...
        when(archiveFragmentActivity.getArchiveFragmentStatePagerAdapter()).thenReturn(
                archiveFragmentStatePagerAdapter);
        when(archiveFragmentStatePagerAdapter.getStatusFilter()).thenReturn(StatusFilter.ALL);

        when(archiveFragmentActivity.getResources()).thenReturn(Robolectric.application.getResources());
    }
    ...
    @Test
    public void getSpinnerElements_NoGridsInDatabase_GridSizeFilterAllReturnedOnly() throws Exception {
        spinner = setSpinnerWithGridSizes();

        String[] gridSizeFilters = {EXPECTED_DESCRIPTION_SIZE_FILTER_ALL};
        assertThat(spinner.getSpinnerElements(), is(Arrays.asList(gridSizeFilters)));
    }

    private ArchiveFragmentGridSizeFilterSpinner setSpinnerWithGridSizes(final GridTypeFilter... gridTypeFilters) {
        return new ArchiveFragmentGridSizeFilterSpinner(archiveFragmentActivity) {
            ...
            AvailableGridTypeFilterSelector createAvailableSizeFilterSelector(StatusFilter statusFilter) {
                ...
                when(availableGridTypeFilterSelector.getAvailableGridTypeFilters()).thenReturn(
            ...
            private List<GridTypeFilter> getGridTypeFiltersIncludingAll() {
                ...
                gridTypeFiltersIncludingAll.addAll(Arrays.asList(gridTypeFilters));
    ...
    @Test
    public void getSpinnerElements_GridsWithMultipleSizesInDatabase_GridSizeFiltersInclusiveAllReturned() throws
            Exception {
        spinner = setSpinnerWithGridSizes(GridTypeFilter.GRID_2X2, GridTypeFilter.GRID_4X4, GridTypeFilter.GRID_5X5);

        String[] gridSizeFilters = {EXPECTED_DESCRIPTION_SIZE_FILTER_ALL, EXPECTED_DESCRIPTION_SIZE_FILTER_2,
                EXPECTED_DESCRIPTION_SIZE_FILTER_4, EXPECTED_DESCRIPTION_SIZE_FILTER_5};
        assertThat(spinner.getSpinnerElements(), is(Arrays.asList(gridSizeFilters)));
    }
    ...
    public void indexOfSelectedGridSizeFilter_GridSizeFilterSelected_AtIndex() throws Exception {
        GridTypeFilter[] gridTypeFilters = {GridTypeFilter.GRID_2X2, GridTypeFilter.GRID_4X4, GridTypeFilter.GRID_5X5};
        ...
        when(archiveFragmentStatePagerAdapter.getSelectedSizeFilter()).thenReturn(lastGridTypeFilter);
        int offsetSetForGridSizeFilterAllAtIndex0 = 1;

        assertThat(spinner.indexOfSelectedGridSizeFilter(),
                   is(offsetSetForGridSizeFilterAllAtIndex0 + indexLastGridSizeFilter));
    }
    ...
    public void testSize_GridsWithMultipleSizesInDatabase_SizeIncludesGridSizeFilterAll() throws Exception {
        ...
        assertThat(spinner.size(), is(countForGridSizeFilterAll + gridTypeFilters.length));
    }
