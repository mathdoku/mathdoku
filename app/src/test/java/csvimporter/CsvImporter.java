...
/**
 * The CsvImporter is the base class to import csv files. This base class handles the reading of lines of data form a
 * csv file. The csv file must adhere to rules below:
 * <p/>
...
public abstract class CsvImporter {
    ...
    public CsvImporter(String fileName, String[] columnNames) {
        if (fileName == null || fileName.isEmpty()) {
            throw new CsvImporterException("Parameter filename should not be empty.");
        }
        if (columnNames == null || columnNames.length == 0) {
            throw new CsvImporterException("At least one column name must be specified.");
        }
        for (String columnName : columnNames) {
            if (columnName == null || columnName.isEmpty()) {
                throw new CsvImporterException(String.format("ColumnName '%s' is not valid.", columnName));
            }
        }
    ...
    /**
     * Read next line from the buffered reader. If the line starts with "//" it is skipped as a comment line.
     *
     * @param bufferedReader
     *         The buffered reader to read from.
     * @return The first line which is not a comment. Null in case of end of stream.
     * @throws java.io.IOException
     */
    protected String getNextRowFromBufferedReader(BufferedReader bufferedReader) throws IOException {
    ...
    public void importFile() {
        ...
        try {
            inputStream = TestRunnerHelper.getActivity()
                    .getAssets()
                    .open(fileName);
            ...
            throw new CsvImporterException(String.format("Cannot import file '%s'.", fileName), e);
        } finally {
            if (br != null) {
                try {
                    br.close();
    ...
    // Subclasses should override this method in case additional work has to be done when entire file is imported
    // successfully.
        ...
        for (String columnName : columnNames) {
            if (index >= values.length || !columnName.equals(values[index])) {
                ...
                throw new CsvImporterException(String.format(
                        ...
                        removeEnclosingQuotationMarks(values[index]), columnName, Arrays.asList(values)
                                .toString(), Arrays.asList(columnNames)
                                .toString()));
            }
            index++;
        }
