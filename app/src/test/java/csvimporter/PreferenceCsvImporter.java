...
/**
 * This class imports records from a csv file into the app preferences. The csv file must adhere to rules below:
 * <p/>
 * 1. Rows starting with "//" are handled as comments and will be ignored. Also empty rows will be ignored.
 * <p/>
 * 2. Columns name, type and value are mandatory and must be defined in this order as the first three columns.
 * <p/>
...
public class PreferenceCsvImporter extends CsvImporter {
    ...
    public PreferenceCsvImporter(String fileName, SharedPreferences sharedPreferences) {
        ...
        try {
            if ("boolean".equalsIgnoreCase(dataType)) {
                ...
                throw new CsvImporterException(
                        String.format("Data type '%s' on line %d is not a valid data type. ", dataType,
                                      getLinesRead()));
            }
        } catch (NumberFormatException e) {
            throw new CsvImporterException(
                    String.format("Value '%s' on line %d is not a valid value for data type '%s'.", value,
                                  getLinesRead(), dataType), e);
        }
    }
