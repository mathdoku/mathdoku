...
public class TipLeaderboardAvailable extends TipDialog {

	public static String TIP_NAME = "LeaderboardAvailable";
	private static TipPriority TIP_PRIORITY = TipPriority.LOW;

	/**
	 * Creates a new tip dialog which explains that the leaderboards have been
	 * made available.
	 * 
	 * @param context
	 *            The activity in which this tip has to be shown.
	 */
	public TipLeaderboardAvailable(Context context) {
		super(context, TIP_NAME, TIP_PRIORITY);

		build(
				context.getResources().getString(
						R.string.dialog_tip_leaderboard_available_title),
				context.getResources().getString(
						R.string.dialog_tip_leaderboard_available_text), null)
				.show();
	}
	...
	public static boolean toBeDisplayed(Preferences preferences) {
		...
		if (preferences.isLeaderboardAvailable() == false) {
			return false;
		}
